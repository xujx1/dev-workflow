...
本 Skill 使用 `writing` 类别模型：

```bash
node .workflow/scripts/model-resolver.js resolve 04-archive
```

记录模型使用：
```bash
node .workflow/scripts/model-resolver.js log 04-archive <model> writing
```
...
用户："归档"
   ...
   └─ [顺序执行]
         ...
         ├─ Step 4-0.6:   知识库保鲜检测（⚠️ freshness check）
         │                 执行 freshness-check.js，检测知识库新鲜度
         │                 - freshness_score >= 80: FRESH，继续归档
         │                 - freshness_score 60-79: STALE_WARN，执行增量 refresh 后继续
         │                 - freshness_score < 60: STALE_BLOCK，阻断归档，要求先执行知识库重建
...
归档阶段读取 `execution-state.md` 中的 `complexity.level`，根据档位决定归档策略：

| 档位 | 归档方式 | OpenSpec 归档 | 知识库更新 | 需求跟踪报告 | Learnings 提取 |
|------|----------|--------------|-----------|-------------|---------------|
| `nano` | 轻量归档 | 跳过 | 跳过 | 不生成 | 跳过 |
| `lite` | 标准归档 | 条件执行 | 执行 | 生成 | 执行 |
| `full` | 完整归档 | 强制执行 | 执行 | 生成（含需求跟踪） | 执行 |

### nano 档位归档

- 仅更新 `execution-state.md` 归档状态
- 不生成 `archive-report.md`
- 跳过知识库更新和 Learnings 提取

### lite 档位归档

- 生成标准 `archive-report.md`（不含需求跟踪矩阵）
- 执行知识库增量更新
- OpenSpec 归档：仅当存在 OpenSpec 时执行
- 执行 Learnings 提取

### full 档位归档

- 生成完整 `archive-report.md`（含需求跟踪矩阵）
- 强制执行 OpenSpec 归档
- 执行知识库完整更新
- 执行 Learnings 提取
- 重建 GitNexus 索引（如已启用）
...
| style-profile 修正 | `.workflow/profiles/style-profile.md`（若有新纠偏） |
| review-profile 修正 | `.workflow/profiles/review-profile.md`（若有新 checkpoint） |
...
### style-profile 修正（每次归档自动执行）

> Review 中出现的纠偏记录自动追加到 style-profile.md

当 `.workflow/profiles/style-profile.md` 存在时：

1. 从本次 `code-review.md` 提取纠偏记录（命名不符、日志缺失、异常未转换等）
2. 若存在新的不符合项 → 追加到 style-profile.md 对应章节
3. 若 review-profile.md 有漏检 checkpoint → 追加到 review-profile.md
